...
train_genes <- scale(as.matrix(train[, 18:5017]))
cv_ridge <- cv.glmnet(
  train_genes, y_train,
  family = "binomial",
  alpha = 1, # Ridge
...
ridge <- glmnet(train_genes, y_train, family = "binomial", alpha = 0, lambda = cv_ridge$lambda.min)
```
...
```{r}
# coef extraction (inclut intercept)
coef_mat <- as.matrix(coef(lasso_final))
names_coef <- rownames(coef_mat)
coef_vec <- as.numeric(coef_mat[, 1])
...
# Prédictions sur le test
yhat_test <- predict(lasso_final, newx = X_test, type = "response")
yhat_class <- ifelse(yhat_test > 0.01, "Dead", "Alive")
table(y_test, yhat_class)
# Taux de précision
mean(yhat_class == y_test)
```
